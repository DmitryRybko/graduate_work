...
user_role = Table(
    ...
    Column(
        'user_id',
        UUID(as_uuid=True),
        ForeignKey(f'{settings.user_table_name}.id', ondelete='CASCADE')
    ),
    Column(
        'role_id',
        UUID(as_uuid=True),
        ForeignKey(f'{settings.role_table_name}.id', ondelete='CASCADE')
    )
)
...
class LogHistory(Base):
    ...
    user_id = Column(
        UUID(as_uuid=True), ForeignKey(f'{settings.user_table_name}.id')
    )
